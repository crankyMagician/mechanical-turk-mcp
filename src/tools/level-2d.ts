...
export function registerLevel2dTools(ctx: ToolContext): ToolRegistration {
  const tools = [
    // --- Headless tools (10) ---
    {
      name: 'modify_node',
  ...
  function validateProjectAndScene(args: any): string | null {
  ...
  const handlers: Record<string, (args: any) => Promise<any>> = {
    ...
    reparent_node: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    create_tilemap: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
      return runHeadlessOp(args, 'create_tilemap', (a) => {
        const params: any = { scenePath: a.scenePath };
        if (a.parentNodePath) params.parentNodePath = a.parentNodePath;
        if (a.nodeName) params.nodeName = a.nodeName;
        if (a.tileSize) params.tileSize = a.tileSize;
        if (a.atlasSource) params.atlasSource = a.atlasSource;
        return params;
      }, `TileMapLayer created in '${args.scenePath}'.`);
    },

    set_tiles: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    add_collision_shape: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
      ...
      return runHeadlessOp(args, 'add_collision_shape', (a) => {
        ...
        if (a.nodeName) params.nodeName = a.nodeName;
        if (a.properties) params.properties = a.properties;
        return params;
      }, `CollisionShape2D added to '${args.parentNodePath}' in '${args.scenePath}'.`);
    },

    configure_physics_body: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    attach_script: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    create_and_attach_script: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
      if (!args.nodePath || !args.scriptPath) {
        return createErrorResponse('nodePath and scriptPath are required');
      }
      ...
      return runHeadlessOp(args, 'create_and_attach_script', (a) => {
        ...
        if (a.baseClass) params.baseClass = a.baseClass;
        if (a.template) params.template = a.template;
        if (a.content) params.content = a.content;
        return params;
      }, `Script created at '${args.scriptPath}' and attached to '${args.nodePath}' in '${args.scenePath}'.`);
    },

    batch_scene_operations: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    // Bridge tools
    set_node_property: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
    ...
    delete_node_live: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
      if (!args.nodePath) {
    ...
    set_tiles_live: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
      if (!args.nodePath || !args.tiles) {
        return createErrorResponse('nodePath and tiles are required');
      }
      try {
        ...
        return createErrorResponse(
          `Failed to set tiles: ${error?.message || 'Unknown error'}`,
          bridgeHints,
        );
      }
    },

    reparent_node_live: async (rawArgs: any) => {
      const args = normalizeParameters(rawArgs || {});
